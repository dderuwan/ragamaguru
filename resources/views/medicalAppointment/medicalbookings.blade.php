            ...
            <div class="row justify-content-center">
                <div class="col-12">
                    <div class="row align-items-center my-3">
                        <div class="col">
                            <h2 class="page-title">Medical Bookings</h2>
                        </div>
                        <div class="col-auto">
...
<script>
    ...
    function loadBookings() {
        ...
        const url = `{{ route('medicalbookings.date', ':date') }}`.replace(':date', date);
                ...
                data.forEach((booking, index) => {
                    ...
                    const row = `
                            <tr>
                                ...
                                <td>${booking.medical_reason}</td>
                                <td>${booking.created_by}</td>
                                <td>${booking.added_date}</td>
                                <td>${booking.status == 0 ? 'Canceled' : 'Active'}</td> 
                                <td>
                                <a href="/m-appointments/add/${booking.customer_id}" class="btn btn-success">
    ...
    function cancelBooking(bookingId) {
        $.ajax({
            url: `/m-bookings/cancel/${bookingId}`,
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}'
            },
